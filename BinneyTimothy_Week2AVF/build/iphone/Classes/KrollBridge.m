...
NSString * BinneyTimothy_Week2AVF$ModuleRequireFormat = @"(function(exports){"
...
@implementation BinneyTimothy_Week2AVFObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_binneytimothy_week2avf);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create BinneyTimothy_Week2AVF global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "BinneyTimothy_Week2AVF" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_binneytimothy_week2avf = [[BinneyTimothy_Week2AVFObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_binneytimothy_week2avf];
	
	NSString *_binneytimothy_week2avfNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _binneytimothy_week2avfNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_binneytimothy_week2avf valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:BinneyTimothy_Week2AVF$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.binneytimothy_week2avf.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.binneytimothy_week2avf.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
