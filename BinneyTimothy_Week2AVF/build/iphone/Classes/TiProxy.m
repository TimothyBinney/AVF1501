...
//Rare exceptions to indicate a bug in the _binneytimothy_week2avf code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_binneytimothy_week2avf = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_binneytimothy_week2avf]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.binneytimothy_week2avf.module"
